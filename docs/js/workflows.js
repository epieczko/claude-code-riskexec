...
function showGenerateModal(hash, yaml) {
    document.getElementById('workflowCommand').textContent = `npx claude-code-riskexec@latest --workflow:#${hash}`;
...
function generateInstallCommand(component) {
    return `npx claude-code-riskexec@latest --${component.type} "${component.path}"`;
}
...
function viewOnGitHub(path) {
    const url = `https://github.com/davila7/claude-code-riskexec/tree/main/${path}`;
