...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  try {
    ...
    if (commandName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/davila7/claude-code-riskexec/main/cli-tool/components/commands/${commandName}.md`;
    } else {
...
async function installIndividualMCP(mcpName, targetDir, options) {
  ...
  try {
    ...
    if (mcpName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/davila7/claude-code-riskexec/main/cli-tool/components/mcps/${mcpName}.json`;
    } else {
...
async function installIndividualSetting(settingName, targetDir, options) {
  ...
  try {
    ...
    if (settingName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/davila7/claude-code-riskexec/main/cli-tool/components/settings/${settingName}.json`;
    } else {
...
async function installIndividualHook(hookName, targetDir, options) {
  ...
  try {
    ...
    if (hookName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/davila7/claude-code-riskexec/main/cli-tool/components/hooks/${hookName}.json`;
    } else {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        if (responseText.includes('rate limit')) {
          ...
          console.log(chalk.yellow('💡 Install locally with: npm install -g claude-code-riskexec'));
...
async function executeSandbox(options, targetDir) {
  ...
  try {
    ...
    const baseUrl = 'https://raw.githubusercontent.com/davila7/claude-code-riskexec/main/cli-tool/components/sandbox/e2b';
