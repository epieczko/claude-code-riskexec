...
async function installIndividualAgent(agentName, targetDir, options) {
  ...
  try {
    ...
    if (agentName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/agents/${agentName}.md`;
    } else {
...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  try {
    ...
    if (commandName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/commands/${commandName}.md`;
    } else {
...
async function installIndividualMCP(mcpName, targetDir, options) {
  ...
  try {
    ...
    if (mcpName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/mcps/${mcpName}.json`;
    } else {
...
async function installIndividualSetting(settingName, targetDir, options) {
  ...
  try {
    ...
    if (settingName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/settings/${settingName}.json`;
    } else {
...
async function installIndividualHook(hookName, targetDir, options) {
  ...
  try {
    ...
    if (hookName.includes('/')) {
      ...
      githubUrl = `https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/hooks/${hookName}.json`;
    } else {
...
async function getAvailableAgentsFromGitHub() {
  try {
    ...
    const response = await fetch('https://api.github.com/repos/riskexec/claude-code/contents/cli-tool/components/agents');
    if (!response.ok) {
    ...
    for (const item of contents) {
      if (item.type === 'file' && item.name.endsWith('.md')) {
        ...
        try {
          const categoryResponse = await fetch(`https://api.github.com/repos/riskexec/claude-code/contents/cli-tool/components/agents/${item.name}`);
...
async function executeSandbox(options, targetDir) {
  ...
  try {
    ...
    const baseUrl = 'https://raw.githubusercontent.com/riskexec/claude-code/main/cli-tool/components/sandbox/e2b';
