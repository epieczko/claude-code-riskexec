...
Object.entries(registry.agents || {}).forEach(([agentName, relativePath]) => {
  ...
  if (attrs.name !== agentName) {
    errors.push(
      `${context} frontmatter name \"${attrs.name}\" does not match registry key`
    );
  }
...
Object.entries(registry.commands || {}).forEach(
  ([commandName, relativePath]) => {
    ...
    requiredCommandFields.forEach((field) => {
      if (!attrs[field]) {
        errors.push(
          `${context} missing required frontmatter field \"${field}\"`
        );
      }
    });
    ...
    if (attrs.command && attrs.command !== `/${commandName}`) {
      errors.push(
        `${context} frontmatter command \"${attrs.command}\" does not match registry key`
      );
    }
    ...
    ['model', 'allowed-tools'].forEach((disallowed) => {
      if (attrs[disallowed]) {
        errors.push(
          `${context} should not define \"${disallowed}\"; configure it on the agent instead`
        );
      }
    });
    ...
    ensureNoExtraKeys(
      attrs,
      ['command', 'description', 'argument-hint', 'agent', 'phase'],
      context
    );
...
if (errors.length > 0) {
  console.error(
    'BMAD component validation failed:\n - ' + errors.join('\n - ')
  );
  process.exit(1);
}
