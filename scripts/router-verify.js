...
if (!fs.existsSync(cfg))
  die('Router config missing at ~/.claude-code-router/config.json', 2);
...
for (const p of json.Providers || []) {
  ...
  if (!allowedProviders.has(p.name))
    die(`Only ${[...allowedProviders]} allowed, found ${p.name}`, 4);
...
for (const k of ['default', 'background', 'think', 'longContext']) {
  ...
  if (!allowedProviders.has(prov))
    die(`Route '${k}' must use provider 'anthropic', found '${prov}'`, 6);
  if (!allowedModels.has(model))
    die(`Route '${k}' model not allowed: ${model}`, 7);
}
