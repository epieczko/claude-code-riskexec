...
export class ImplementPhase implements PhaseHandler {
  ...
  public async run(options: PhaseRunOptions): Promise<PhaseResult> {
    ...
    const storedTaskContext: TaskContext | null =
      options.taskContext ??
      (await loadContext<TaskContext>(options.featureName, 'tasks', {
    ...
    if (!tasks.length) {
      throw new Error(
        'No tasks found in tasks.md. Ensure the file contains checklist items.'
      );
    }

    let startIndex = 0;
    if (options.resumeFromTask) {
      const resumeIndex = tasks.findIndex((task) =>
        task.title.includes(options.resumeFromTask as string)
      );
      if (resumeIndex === -1) {
        throw new Error(
          `Unable to locate task containing "${options.resumeFromTask}".`
        );
      }
    ...
    for (let i = startIndex; i < tasks.length; i += 1) {
      ...
      logger.info(
        `Task ${task.index}/${tasks.length} complete → ${path.basename(outputPath)}`
      );
    }
