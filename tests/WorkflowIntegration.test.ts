...
import { makeSpecContext, makeSpecMarkdown, makeTasksMarkdown } from './factories';
...
const recordMetricsMock = recordMetrics as jest.MockedFunction<typeof recordMetrics>;
...
describe('Workflow integration', () => {
  ...
  it('runs all phases sequentially and records analytics metrics', async () => {
    ...
    expect(metrics.details?.phases).toEqual(['specify', 'plan', 'tasks', 'implement']);
  ...
  it('supports resuming from a specified phase with existing artifacts', async () => {
    ...
    const results = await runWorkflow({ workspaceRoot, feature, resumeFrom: 'plan' });
    expect(results.map(result => result.phase)).toEqual(['plan', 'tasks', 'implement']);
  ...
  it('resumes implementation from a specific task label', async () => {
    ...
    invokeAgentMock.mockImplementation(async options => {
      switch (options.agent) {
        ...
        case 'bmad-architect':
          return { ok: true, outputText: '# Plan\n\n## Architecture Decisions\n- Step\n', command: 'plan' };
        case 'bmad-pm':
          return {
            ok: true,
            outputText: makeTasksMarkdown([
        ...
        case 'bmad-developer':
          return { ok: true, outputText: `Notes for ${options.metadata?.taskLabel}`, command: 'impl' };
        default:
    ...
    const developerCalls = invokeAgentMock.mock.calls.filter(call => call[0].agent === 'bmad-developer');
  ...
  it('records failed runs when a phase throws an error', async () => {
    ...
    await expect(runWorkflow({ workspaceRoot, feature })).rejects.toThrow('agent failure');
