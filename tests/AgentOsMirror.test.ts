...
describe('mirrorAgentOsDirectory phase artifacts', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(featureDir, 'implementation'), {
      recursive: true,
    });
    ...
    await fs.writeFile(
      path.join(featureDir, 'implementation', 'task-1.md'),
      '# Log 1\n',
      'utf8'
    );
    await fs.writeFile(
      path.join(featureDir, 'implementation', 'task-2.md'),
      '# Log 2\n',
      'utf8'
    );
    await fs.mkdir(path.join(featureDir, 'context'), { recursive: true });
    await fs.writeFile(
      path.join(featureDir, 'context', 'tasks.json'),
      '{"tasks": []}',
      'utf8'
    );
  });
  ...
  it('mirrors spec, plan, tasks, and implementation logs into Agent OS root', async () => {
    ...
    await expect(
      fs.readFile(path.join(productRoot, 'spec.md'), 'utf8')
    ).resolves.toBe('# Spec\n');
    await expect(
      fs.readFile(path.join(productRoot, 'plan.md'), 'utf8')
    ).resolves.toBe('# Plan\n');
    await expect(
      fs.readFile(path.join(productRoot, 'tasks.md'), 'utf8')
    ).resolves.toBe('# Tasks\n- [ ] Item\n');

    const implementationFiles = await fs.readdir(
      path.join(productRoot, 'implementation')
    );
    expect(implementationFiles.sort()).toEqual(['task-1.md', 'task-2.md']);
    const logOne = await fs.readFile(
      path.join(productRoot, 'implementation', 'task-1.md'),
      'utf8'
    );
    const logTwo = await fs.readFile(
      path.join(productRoot, 'implementation', 'task-2.md'),
      'utf8'
    );
