...
describe('phase registry', () => {
  it('includes all Spec Kit phases with correct agents', () => {
    expect(Object.keys(phaseRegistry).sort()).toEqual(['implement', 'plan', 'specify', 'tasks', 'verify']);
    ...
    expect(phaseRegistry.verify).toMatchObject({ agent: 'bmad-qa', output: 'specs/{feature}/qa-report.md' });
  });
  ...
  it('provides downstream file dependencies for coordination', () => {
    ...
    expect(phaseRegistry.verify?.input).toEqual(['spec.md', 'plan.md', 'tasks.md', 'implementation']);
  });
});
