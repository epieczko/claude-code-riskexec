...
describe('Workflow telemetry', () => {
  ...
  beforeEach(async () => {
    jest.clearAllMocks();
    workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'workflow-telemetry-'));
  ...
  it('records telemetry entries for each successful phase', async () => {
    ...
    const statusPath = path.join(workspaceRoot, '.agent-os', 'product', 'status.json');
    ...
    expect(telemetry.map((entry: any) => entry.phase)).toEqual(results.map(result => result.phase));
  ...
  it('records failure telemetry when a phase throws', async () => {
    ...
    await expect(runWorkflow({ workspaceRoot, feature })).rejects.toThrow('phase failure');
